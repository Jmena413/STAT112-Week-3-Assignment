...
```{r, fig.alt="Faceted density plots showing that bike rentals happen more often over the weekend by casual clients."}
Trips %>%
  mutate(time = hour(sdate) + (minute(sdate) / 60)) %>%
  mutate(week_day = wday(sdate, label = TRUE)) %>%
  ggplot(aes(x = time, fill = client,)) +
...
```{r, fig.alt="Density plot showing the distribution of when bike rentals happen most often based a client's status. Most weekend rentals happen at midday by casual renters. During the weekdays, registered clients have peak renting hours in the morning and at night."}
Trips %>%
  mutate(time = hour(sdate) + (minute(sdate) / 60)) %>%
  mutate(week_day = wday(sdate, label = TRUE)) %>%
...
```{r, fig.alt="Density plots showing the distribution of bike rentals on each day of the week based on client status."}
Trips %>%
  mutate(time = hour(sdate) + (minute(sdate) / 60)) %>%
  mutate(week_day = wday(sdate, label = TRUE)) %>%
  mutate(weekend = ifelse(week_day == "Sun" | week_day == "Sat", "weekend", "weekday")) %>%
...
```{r, fig.alt="Scatterplot showing the distribution of bike rentals based on latitude and longitude."}
Stations %>%
...
```{r, fig.alt="Scatter plot showing the density and usage rate by client status of bike rentals based on latitude and longitude."}
Stations %>%
  left_join(Trips, by = c("name" = "sstation")) %>%
  group_by(name) %>%
  mutate(week_day = wday(sdate, label = TRUE)) %>%
...
```{r, fig.height=40, fig.width=8, fig.alt="Dot plot showing rating score for each breed of dog reported"}
breed_traits_total <-
  breed_traits
